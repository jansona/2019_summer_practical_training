package com.babyback.api.admin.biz.controller;
...
import com.babyback.api.common.constant.enums.CommonEnums;
import com.babyback.api.common.controller.BaseController;
...
public class SysUserController extends BaseController {
    ...
    @GetMapping("/infoTel")
    @ApiOperation(value = "获取当前授权用户信息", notes = "必须经过了OAuth授权")
    public Result<SysUser> infoTel() {
        String username = SecurityUtils.getUsername();
        SysUser user = sysUserService.findByTelOrEmail(username);
        if (user == null) {
            return new Result<>(CommonEnums.USER_ERROR);
        }
        return new Result<>(user);
    ...
    public Result<SysUser> info(@PathVariable("username") String username) {
        Result<SysUser> sysUserResult = new Result<>(sysUserService.findByName(username));
        return sysUserResult;
    }

    @GetMapping("/infoTel/{username}")
    @ApiOperation(value = "根据电话或Email查询用户信息")
    @ApiImplicitParam(name = "username", value = "用户名", required = true, dataType = "String")
    public Result<SysUser> infoTel(@PathVariable("username") String username) {
        Result<SysUser> sysUserResult = new Result<>(sysUserService.findByTelOrEmail(username));
        return sysUserResult;
    }
